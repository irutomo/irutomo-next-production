// japan_infoテーブルをSupabaseに作成するスクリプト
...
// japan_infoテーブルが存在するかチェックする関数
async function checkJapanInfoTableExists() {
  try {
    // japan_infoテーブルに対して簡単なクエリを実行して存在確認
    ...
    if (error) {
      if (error.code === '42P01') {
        // テーブルが存在しない場合（PostgreSQLのエラーコード）
        console.log('japan_infoテーブルが存在しません。作成します。');
        return false;
      } else {
        // その他のエラー
    ...
    console.log('japan_infoテーブルは既に存在します。');
    return true;
  } catch (err) {
    console.error('テーブル確認中の例外:', err);
    return false;
  }
}
...
CREATE INDEX IF NOT EXISTS idx_japan_info_tags ON public.japan_info USING GIN (tags);
CREATE INDEX IF NOT EXISTS idx_japan_info_is_popular ON public.japan_info (is_popular);
CREATE INDEX IF NOT EXISTS idx_japan_info_published_at ON public.japan_info (published_at);
...
async function createJapanInfoTable() {
  console.log('japan_infoテーブルを作成します...');
  try {
    // execute_sql RPCを利用してSQLを実行する
    ...
    if (error) {
      ...
      // RPCが失敗した場合、代替方法を提案
      console.log('RPCが失敗しました。以下のSQLをSupabase管理画面のSQLエディタで実行してください:');
      console.log(createTableSQL);
      
      throw new Error(`テーブル作成中のエラー: ${error.message}`);
    }
    
    console.log('japan_infoテーブルが正常に作成されました。');
    return true;
  } catch (err) {
    console.error('テーブル作成中のエラー:', err);
    
    // 最終的な手段として、SQLを表示
    console.log('以下のSQLをSupabase管理画面のSQLエディタで実行してください:');
    console.log(createTableSQL);
...
도쿄의 대중교통은 매우 발달되어 있으며, 스이카나 파스모와 같은 카드를 사용하면 편리합니다.
...
// メイン関数
async function main() {
  console.log('japan_infoテーブル作成スクリプトを開始します...');
  ...
  if (!tableExists) {
    // テーブルが存在しない場合は作成
    const tableCreated = await createJapanInfoTable();
    
    if (!tableCreated) {
      console.error('japan_infoテーブルの作成に失敗しました。');
      process.exit(1);
    }
    ...
    console.log('japan_infoテーブルは既に存在します。何もする必要はありません。');
  }
  
  console.log('japan_infoテーブル作成スクリプトが完了しました。');
}
