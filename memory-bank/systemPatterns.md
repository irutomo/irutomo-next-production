...
1. **App Routerディレクトリ構造**
   ...
   - `app/japan-info/` - 日本情報ページ (Strapiと連携)
...
2. **コンポーネント構造**
   ...
   - `lib/` - ユーティリティ関数
   - `lib/strapi/` - Strapi関連のユーティリティとクライアント
   - `hooks/` - カスタムフック
   - `contexts/` - コンテキスト
...
4. **外部CMSデータ統合**
   - Strapi CMSからのデータフェッチ
   - データ構造のマッピングと変換
   - 複数データソースの統合(Supabase + Strapi)
   - キャッシュと再検証戦略
...
3. **テーブル設計パターン**
   ...
   - 支払い関連テーブル
     ...
     - `transactions` - 取引履歴
   - 日本情報関連テーブル
     - `japan_info` - 日本情報基本データ
     - `japan_info_images` - 関連画像
     - `japan_info_tags` - タグ情報
...
## ヘッドレスCMS (Strapi) パターン

### アーキテクチャ
1. **分離型CMS**
   - Next.jsとStrapiを分離してデプロイ
   - Next.js: Vercelにデプロイ
   - Strapi: Railwayにデプロイ
   - APIを通じた連携
...
2. **データフロー**
   - StrapiでコンテンツをCRUD操作
   - Next.jsアプリからAPIを使用してコンテンツ取得
   - データ変換・マッピングレイヤーを実装
   - 複数のデータソース（SupabaseとStrapi）の統合表示

### 連携パターン
1. **APIクライアント**
   - `/lib/strapi/client.ts` - 専用クライアントの実装
   - 型安全なAPI呼び出し
   - エラーハンドリングとリトライロジック
   - データ構造マッピング関数

2. **環境変数管理**
   - `NEXT_PUBLIC_STRAPI_URL` - StrapiのURL
   - `STRAPI_API_TOKEN` - APIアクセストークン
   - 環境ごとの設定分離

3. **コンテンツタイプ設計**
   - `japan-info-articles` - 日本情報記事
   - 多言語フィールド（日本語・韓国語）
   - メディア管理（画像複数枚）
   - メタデータとSEO情報
...
### Railwayデプロイパターン
1. **環境設定**
   - Railway CLI (`railway up --service Strapi`)
   - 環境変数設定 (`railway variables --set "KEY=VALUE"`)
   - ポート設定 (`PORT=1337`)
   - ホスト設定 (`HOST=0.0.0.0`)

2. **運用モード**
   - 本番モード (`NODE_ENV=production`) - 安定動作、コンテンツ編集のみ
   - 開発モード (`NODE_ENV=development`) - コンテンツタイプ編集可能、不安定

3. **ヘルスチェック対応**
   - Railwayのヘルスチェック設定の調整
   - 専用エンドポイントの作成検討
   - 適切なステータスコード設定
...
### マルチデータソース統合パターン
- SupabaseとStrapiからのデータを統合して表示
- 各データソースの特性を活かした使い分け (Supabase: ユーザーデータ、予約、決済 / Strapi: コンテンツ)
- データマッピングレイヤーによる構造変換
- フォールバックとキャッシュ戦略 